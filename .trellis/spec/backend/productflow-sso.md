...
- Scope: `GET /api/productflow/sso/start`, `POST /api/productflow/sso/verify`, Atelier token create-or-reuse behavior,
  New API image/text model option propagation, sidebar entry wiring, and the relay correlation id used by Atelier usage
  records.
- Out of scope: provider channel selection, quota settlement internals, and Atelier tenant storage.
...
- `GET /api/health/sso` (ProductFlow side, public)
  ...
  - Rate-limited via slowapi at 6 requests per minute per client IP to
    deter scanning.
- `X-Atelier-Request-Id` on relay requests
  - Optional request header supplied by Atelier for model calls that should be
    correlated back to Atelier usage/audit records.
  - The value is an opaque id generated by Atelier, maximum 64 characters,
    using only URL-safe alphanumerics plus `_` and `-`.
  - New API persists the validated id on consume/error logs and lets admin/self
    log APIs filter by `atelier_request_id`.
  - This id is not an authentication credential and must not affect routing,
    billing, quota settlement, or channel selection.
- `GET /api/log/token?atelier_request_id=<id>`
  - Token-authenticated read-only endpoint guarded by `TokenAuthReadOnly()`.
  - Returns the authenticated token's recent logs, optionally filtered by
    `atelier_request_id`.
  - Preserves the actual log `id` so Atelier can copy New API billing facts into
    its local `audit_events` row.
...
- `X-Atelier-Request-Id` is safe to log after validation, but raw invalid
  header values must not be echoed into logs or API errors.

Relay usage-record contract:

- New API is the billing source of truth for Atelier model calls.
- Atelier may display quota/cost facts copied from New API logs, but must not
  recalculate final prices independently.
- Consume and error logs should include the validated `atelier_request_id`
  whenever the relay request supplied one.
- Log filtering by `atelier_request_id` must preserve existing user scoping:
  self log APIs only return the authenticated user's logs, and admin log APIs
  remain admin-only.
- Token log lookup by `atelier_request_id` must preserve token scoping: it
  returns only rows whose `token_id` equals the authenticated token id. It must
  not fall back to user-wide logs or admin-wide logs.
...
- Invalid `X-Atelier-Request-Id` on a relay request -> do not persist the raw
  value. The relay request may continue without correlation or return a safe
  400 according to the implementation choice, but it must never panic or leak
  the header value into logs.
- Log query with `atelier_request_id` and no matching row -> normal empty page
  result, not an error.
- Token log query with `atelier_request_id` for a log owned by another token ->
  normal empty result, not an authorization bypass.
...
- Relay consume/error log tests cover valid `X-Atelier-Request-Id`
  persistence and filtering.
- Token log lookup tests cover `GET /api/log/token?atelier_request_id=...`
  returning only logs owned by the authenticated token while keeping the
  real log `id`.
- Existing log queries without `atelier_request_id` keep their previous
