...
Make New API the source of truth for the image-generation models available to Atelier SSO sessions, while the active
image model is chosen by the signed-in Atelier user in **生成设置**.
...
Atelier must consume the SSO-provided model list for interactive image generation and expose it as a personal dropdown in
the image-chat **生成设置** panel. The selected model is submitted with that generation and snapshotted for retries.
Atelier `/settings` provider bindings remain fallback/bootstrap configuration and must not decide the active SSO image
model.
...
|---|----------|----------|--------|
| 1 | Where does admin define available image models? | New API token group/channel config | Group membership and channel availability live in New API. |
| 2 | Where does the active image model get chosen? | Atelier image-chat **生成设置** per user/session | Different users/groups can have different model options, and the choice belongs to the generation request. |
...
### R4 - SSO verify carries image model options
...
### R5 - Atelier consumes SSO model options for image generation

- Store the returned `token_group`, default `image_model`, and `image_models` on the server-side auth session.
...
- Local provider binding image model remains the fallback for non-SSO/bootstrap sessions only. Token-backed SSO sessions
  that do not carry model options must fail with a safe re-entry/configuration message instead of silently using a stale
  local model.
...
- [ ] SSO verify payload includes `token_group`, optional `image_model`, and `image_models`.
